...
bool BHController::Init(OpenRAVE::RobotBasePtr robot, std::vector<int> const &dof_indices, int ctrl_transform)
{
    ...
    srv_movehand_ = nh_bhd.serviceClient<owd_msgs::MoveHand>("MoveHand");
    return true;
}
                               ...
                               OpenRAVE::TransformConstPtr transform)
{
    ...
    for (size_t i = 0; i < num_dofs; ++i) {
        request.positions[dof_indices_[i]] = values[i];
    }

    owd_msgs::MoveHand::Response response;
    bool const success = srv_movehand_.call(request, response) && response.ok;
...
{
    throw OpenRAVE::openrave_exception("SetPath is not implemented for the BarrettHand.", OpenRAVE::ORE_NotImplemented);
}
